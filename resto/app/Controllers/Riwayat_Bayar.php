...
class Riwayat_Bayar extends Controller
{
   ...
   public function cart_riwayat_bayar($ref_bayar = 0)
   {
      ...
      $data['ref_bayar'] = $this->db(0)->get_where('kas', "ref_bayar = '" . $ref_bayar . "'", "ref", 1);
      ...
      $data['order'] = $this->db(0)->get_where('ref', $where, "tgl", 1);
      $data['order_ref'] = $this->db(0)->get_where('ref', $where, "id");
      ...
      foreach ($data['order_ref'] as $key => $r) {
         ...
         $cek_dibayar[$key] = $this->db(0)->get_where('kas', "status_mutasi <> 2 AND jenis_transaksi = 1 AND ref_bayar <> '' AND ref = '" . $key . "'");
   ...
   function batalkan()
   {
      ...
      $up = $this->db(0)->update('ref', 'step = 3', $where);
      if ($up['errno'] == 0) {
         $del = $this->db(0)->delete_where('kas', "ref_bayar = '" . $ref_bayar . "'");
