...
class Stok extends Controller
{
   ...
   public function index()
   {
      ...
      for ($i = 0; $i >= -6; $i--) {
         ...
         $data['qty'][$tgl]['a'] = $this->db(db(0))->sum_col_where('stok', 'a', "tgl = '" . $tgl . "' AND a <> 0");
         $data['qty'][$tgl]['s'] = $this->db(db(0))->sum_col_where('stok', 's', "tgl = '" . $tgl . "' AND s <> 0");
         $tgl_pesan = date('Y-m-d', strtotime($i . ' days', strtotime(date('Y-m-d'))));
         $terjual = $this->db(db(0))->get_cols_where('pesanan', 'id_menu, SUM(qty) as qty', "insertTime LIKE '" . $tgl_pesan . "%' GROUP BY id_menu", 1, 'id_menu'); //sale
   ...
   function cek($get, $mode = "a")
   {
      ...
      if ($mode == "sa") {
         ...
         $data['data'] = $this->db(db(0))->get_where('stok', "tgl = '" . $get . "'", "id_menu");
      ...
      if ($p['tgl'] == date('Ymd')) {
         foreach ($p['data'] as $key => $d) {
            if ($d <> '') {
               ...
               $update = $mode . " = " . $d;
               $in = $this->db(db(0))->insertCols("stok", $cols, $vals, $update);
               if ($in['errno'] <> 0) {
                  echo $in['error'];
                  exit();
               }
         ...
         echo $this->db(db(0))->sum_col_where('stok', $mode, "tgl = '" . $p['tgl'] . "' AND " . $mode . " <> 0");
      } else {
         echo "date expired";
      }
   }
