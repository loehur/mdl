...
import { ChevronRight, LockKeyhole, LogOut, Moon, Smartphone, Trash2 } from 'lucide-vue-next';
...
<template>
  <AppShell><div class="page"><header><p class="text-sm font-medium text-nemu-600 dark:text-[#83d8b2]">Preferences</p><h1 class="mt-2 text-3xl font-semibold tracking-[-.035em]">Settings</h1></header>
    ...
    <section class="mt-8"><p class="px-1 text-xs font-semibold uppercase tracking-[.12em] text-muted">Data</p><div class="surface mt-3 overflow-hidden rounded-2xl border border-line"><RouterLink to="/trash" class="flex items-center gap-3 p-4"><Trash2 :size="19" class="text-muted"/><span class="flex-1 font-medium">Trash</span><ChevronRight :size="19" class="text-muted"/></RouterLink></div></section><button class="mt-9 flex w-full items-center justify-center gap-2 rounded-2xl py-3 text-sm font-medium text-red-600 transition hover:bg-red-50 dark:text-red-300 dark:hover:bg-red-950/25" @click="logout"><LogOut :size="17"/>Keluar dari akun</button><p class="mt-4 text-center text-xs leading-5 text-muted">Memory kamu bersifat pribadi.<br/>NEMU v0.1</p>
  </div></AppShell>
  <Transition name="fade"><div v-if="upgradeNotice" class="fixed inset-0 z-40 flex items-end bg-black/40 p-4 sm:items-center sm:justify-center" @click.self="upgradeNotice = false"><section role="dialog" aria-modal="true" aria-labelledby="upgrade-title" class="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl dark:bg-[#202721]"><span class="grid size-11 place-items-center rounded-2xl bg-nemu-50 text-nemu-600 dark:bg-nemu-500/20 dark:text-[#a9edcb]"><Smartphone :size="21"/></span><h2 id="upgrade-title" class="mt-5 text-xl font-semibold tracking-[-.02em]">Upgrade melalui aplikasi Android</h2><p class="mt-3 text-sm leading-6 text-muted">Untuk keamanan pembayaran, upgrade ke Basic atau Pro hanya dapat dilakukan melalui aplikasi NEMU di Google Play. Setelah pembayaran dikonfirmasi, plan kamu akan diperbarui otomatis di semua perangkat.</p><button class="primary-button mt-6 w-full" @click="upgradeNotice = false">Mengerti</button></section></div></Transition>
</template>
