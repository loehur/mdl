...
import { planService, type Plan, type PlanInfo } from '@/services/plan-service';
...
const activePlan = ref<PlanInfo>(); const changingPlan = ref<Plan | null>(null);
...
async function choosePlan(plan: Plan) { changingPlan.value = plan; try { activePlan.value = await planService.set(plan); } finally { changingPlan.value = null; } }
</script>
...
<template>
  <AppShell><div class="page"><header><p class="text-sm font-medium text-nemu-600 dark:text-[#83d8b2]">Preferences</p><h1 class="mt-2 text-3xl font-semibold tracking-[-.035em]">Settings</h1></header>
    ...
    <section class="mt-8"><div class="flex items-center justify-between px-1"><p class="text-xs font-semibold uppercase tracking-[.12em] text-muted">Plan</p><span v-if="activePlan" class="text-xs font-semibold text-nemu-600 dark:text-[#a9edcb]">{{ activePlan.used }}/{{ activePlan.limit }} memories</span></div><div class="mt-3 grid gap-2"><button v-for="item in [{ id: 'free', name: 'Free', limit: 100 }, { id: 'personal', name: 'Personal', limit: 1000 }, { id: 'pro', name: 'Pro', limit: 5000 }]" :key="item.id" class="surface flex items-center rounded-2xl border p-4 text-left transition" :class="activePlan?.plan === item.id ? 'border-nemu-500 ring-1 ring-nemu-500/30' : 'border-line hover:border-nemu-500/50'" :disabled="!!changingPlan" @click="choosePlan(item.id as Plan)"><span class="flex-1"><span class="block font-semibold">{{ item.name }}</span><span class="mt-1 block text-sm text-muted">{{ item.limit.toLocaleString() }} memory</span></span><span v-if="activePlan?.plan === item.id" class="rounded-full bg-nemu-50 px-2 py-1 text-xs font-semibold text-nemu-600 dark:bg-nemu-500/20 dark:text-[#a9edcb]">Aktif</span></button></div><p class="mt-2 px-1 text-xs text-muted">Plan statis untuk saat ini; payment gateway belum aktif.</p></section>
