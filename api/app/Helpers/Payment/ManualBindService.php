...
class ManualBindService
{
    ...
    { $db=$db?:DB::getInstance(0); $r=$db->query("SELECT id FROM payment_manual_binds WHERE status='pending' AND payment_method=? AND amount = ? LIMIT 1",[$method,$amount])->row_array(); if ($r) return false; if ($method==='qris') { $r=$db->query("SELECT id FROM qris_nominal_reservations WHERE state='pending' AND amount = ? LIMIT 1",[$amount])->row_array(); return empty($r); } try { foreach (BcaUniqueNominal::collectUsedAmounts(DB::getInstance(6), DB::getInstance(4), DB::getInstance(1), DB::getInstance(2)) as $used) if ($amount === (int)$used) return false; } catch (\Throwable $e) {} return true; }
}
