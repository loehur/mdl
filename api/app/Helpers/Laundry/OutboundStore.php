...
class OutboundStore
{
     ...
     * @return array{ok:bool,message?:string,cooldown?:bool,outcome?:string}
     */
    public static function sendStatus(array $input): array
    {
        $waNumber = trim((string) ($input['wa_number'] ?? $input['phone'] ?? ''));
        $custId = (int) ($input['cust_id'] ?? $input['id_pelanggan'] ?? 0);
        ...
        $replies = new \App\Models\WAReplies();
        return $replies->sendStatusFromCrm($waNumber, $custId > 0 ? $custId : null);
    }
     ...
     * @return array{ok:bool,message?:string,cooldown?:bool,outcome?:string}
     */
    public static function sendQris(array $input): array
    {
        $waNumber = trim((string) ($input['wa_number'] ?? $input['phone'] ?? ''));
        $custId = (int) ($input['cust_id'] ?? $input['id_pelanggan'] ?? 0);
        ...
        $replies = new \App\Models\WAReplies();
        return $replies->sendQrisFromCrm($waNumber, $custId > 0 ? $custId : null);
    }
}
