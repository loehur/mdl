...
class Templates extends WaDeskController
{
    ...
    public function deleteForTeam()
    {
        ...
        if (!$this->isPost()) $this->error('Method not allowed', 405);
        if (!in_array((string) ($user['role'] ?? ''), ['admin', 'team_leader'], true) || !$this->hasOperationalTeam($user)) $this->error('Hanya Admin atau Team Leader pada team aktif yang dapat menghapus template.', 403);
