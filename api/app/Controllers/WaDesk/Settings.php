...
class Settings extends WaDeskController
{
    ...
    /** @deprecated Limit harian hanya di tenant — endpoint ini tidak dipakai lagi. */
    public function wabaDailyLimit()
    {
        $this->verifyAuth();
        $this->requireAdmin();
        $this->error('Limit harian hanya diatur per tenant (Config → Default limit harian).', 410);
    }
