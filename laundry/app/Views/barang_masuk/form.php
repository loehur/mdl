...
<div class="border-top pt-3 mt-3">
  ...
  <div id="cartTotal" class="d-none border-top pt-2 mt-2">
    ...
    <button type="button" id="btnCheckout" class="btn btn-success w-100">
      <i class="fas fa-check-circle me-1"></i> Simpan Barang Masuk
    </button>
  </div>
</div>
...
$(document).ready(function() {
  ...
  $('#selectBarang').on('change', function() {
    ...
    if (id) {
      $.ajax({
        url: '<?= URL::BASE_URL ?>BarangMasuk/get_sub/' + id,
        type: 'GET',
        dataType: 'json',
  ...
  $(document).on('click', '.addToCartMainNew', function() {
    ...
    $.ajax({
      url: '<?= URL::BASE_URL ?>BarangMasuk/add_main_to_cart',
      type: 'POST',
  ...
  $(document).on('click', '.removeFromCart', function() {
    ...
    $.ajax({
      url: '<?= URL::BASE_URL ?>BarangMasuk/remove_from_cart',
      type: 'POST',
      data: { key: key },
      dataType: 'json',
      success: function(res) {
  ...
  $(document).on('click', '#btnCheckout', function() {
    ...
    showConfirmModal('Simpan sebagai Barang Masuk?', function() {
      btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Processing...');
      
      $.ajax({
        url: '<?= URL::BASE_URL ?>BarangMasuk/checkout',
        type: 'POST',
        dataType: 'json',
        success: function(res) {
...
function addToCart(id_barang, id_sub, qty) {
  $.ajax({
    url: '<?= URL::BASE_URL ?>BarangMasuk/add_to_cart',
    type: 'POST',
...
function loadCart() {
  ...
  $.ajax({
    url: '<?= URL::BASE_URL ?>BarangMasuk/cart',
    type: 'GET',
    cache: false,
  ...
  <div class="modal-dialog modal-dialog-centered modal-sm">
    <div class="modal-content border-0 shadow">
      ...
      <div class="modal-body text-center py-4">
        ...
        <p id="confirmModalMessage" class="mb-0">Simpan Barang Masuk?</p>
      </div>
