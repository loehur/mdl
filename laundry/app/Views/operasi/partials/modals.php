...
<div class="op-modal" id="modalUbahQty" aria-hidden="true">
  <div class="op-modal__backdrop" data-op-close></div>
  <div class="op-modal__panel op-modal__panel--form" role="dialog" aria-modal="true" aria-labelledby="opQtyTitle">
    <div class="op-modal__head op-modal__head--blue">
      <div>
        <h3 id="opQtyTitle">Ubah Quantity</h3>
        <small>Ganti qty item (belum tuntas, tanpa overpay)</small>
      </div>
      <button type="button" class="op-modal__close" data-op-close aria-label="Tutup"><i class="fas fa-times"></i></button>
    </div>
    <div class="op-modal__body">
      <div id="ubahQtyLoading" class="op-loading">
        <i class="fas fa-spinner fa-spin"></i> Memuat...
      </div>
      <div id="ubahQtyContent" class="d-none">
        <p class="op-muted" id="ubahQtyInfo"></p>
        <div class="op-field"><strong>Item:</strong> <span id="ubahQtyItem"></span></div>
        <div class="op-field">
          <label class="op-label" for="ubahQtyInput">Quantity baru</label>
          <div style="display:flex;align-items:center;gap:8px;">
            <input type="number" id="ubahQtyInput" class="op-input" min="0.01" step="0.01" inputmode="decimal" style="flex:1;">
            <span id="ubahQtySatuan" class="op-muted"></span>
          </div>
          <small id="ubahQtyMinHint" class="op-muted d-none" style="display:block;margin-top:4px;"></small>
        </div>
        <div class="op-summary">
          <div>Harga item: <strong id="ubahQtyItemHarga">-</strong></div>
          <div>Total order: <strong id="ubahQtyRefTotal">-</strong></div>
          <div id="ubahQtyBayarInfo" class="d-none" style="color:#d97706;margin-top:6px;">Pembayaran Cek/Berhasil: <strong id="ubahQtyDibayar">-</strong></div>
        </div>
        <div id="ubahQtyAlert" class="op-alert d-none"></div>
      </div>
    </div>
    <div class="op-modal__foot">
      <button type="button" class="op-btn op-btn--ghost" data-op-close>Batal</button>
      <button type="button" id="btnSimpanQty" class="op-btn op-btn--primary" disabled>Simpan</button>
    </div>
  </div>
</div>
...
(function () {
  ...
  function syncPendingUi() {
    var jenis = String(document.getElementById('kurirJenis').value || '').toLowerCase();
    var wrap = document.getElementById('kurirLangsungAntarWrap');
    var cb = document.getElementById('kurirLangsungAntar');
    if (!wrap || !cb) return;
    var show = !forceComplete && (jenis === 'antar' || jenis === 'jemput_antar');
    ...
    cb.disabled = ignore;
    if (show && wasHidden) {
      // Default: tidak dicentang = pending
      cb.checked = false;
    }
  }

  /** true = pending (parkir); false = langsung ke board / selesai */
  function wantPendingAntar() {
    if (forceComplete) return false;
    var cb = document.getElementById('kurirLangsungAntar');
    if (!cb || cb.disabled) return false;
    // Tidak dicentang = pending
    return !cb.checked;
  }
  ...
  function syncSubmitLabel() {
    ...
    if (jenis === 'jemput_antar') {
      lab.textContent = wantPendingAntar()
        ? 'Selesai Jemput + Request Antar Pending'
        : 'Selesai Jemput + Request Antar';
      return;
    }
    if (jenis === 'jemput') {
    ...
    lab.textContent = wantPendingAntar() ? 'Buat Request Pending' : 'Buat Request';
  }
  ...
  function renderSales(orders) {
    ...
    box.innerHTML = orders.map(function (ord) {
      var items = (ord.items || []).map(function (it) {
        ...
        var surcasMine = !!it.surcas_mine;
        var terikatSurcasLain = !!it.terikat_surcas_lain;
        var meta = '#' + it.id + (it.member ? ' · member' : '')
          ...
          + (surcasMine ? ' · item surcas ini' : '')
          + (terikatSurcasLain ? ' · terikat surcas lain' : '')
        ...
        return '<label class="kurir-item">' +
          ...
          (surcasMine ? ' data-surcas-mine="1"' : '') +
          (terikatSurcasLain ? ' data-terikat-surcas-lain="1"' : '') +
  ...
  function lockBoundCheckboxes() {
    ...
    root.querySelectorAll('input[name="kurir_ids"]').forEach(function (cb) {
      var bound = false;
      if (cb.getAttribute('data-terikat-surcas-lain') === '1') {
        bound = true;
      }
      if (cb.getAttribute('data-terikat') === '1') {
        bound = true;
      }
      ...
      if (!forceComplete) {
        ...
        if (lockJemput && cb.getAttribute('data-surcas-jemput') === '1') bound = true;
        if (lockAntar && cb.getAttribute('data-surcas-antar') === '1') bound = true;
      }
      if (!bound) return;
      cb.checked = true;
      cb.disabled = true;
  ...
  function applyPrefillRef() {
    if (!prefillNoRef) return;
    var want = String(prefillNoRef);
    root.querySelectorAll('.kurir-group').forEach(function (g) {
      ...
      g.querySelectorAll('input[name="kurir_ids"]').forEach(function (cb) {
        ...
        if (forceComplete) {
          var jenisFc = String((document.getElementById('kurirJenis') || {}).value || '').toLowerCase();
          if (jenisFc === 'jemput') {
            cb.checked = match
              && (cb.getAttribute('data-surcas-mine') === '1' || cb.getAttribute('data-surcas-jemput') === '1')
              && cb.getAttribute('data-sudah-delivered') !== '1';
          } else {
            cb.checked = match
              && (cb.getAttribute('data-surcas-mine') === '1' || cb.getAttribute('data-surcas-antar') === '1')
              && cb.getAttribute('data-sudah-delivered') !== '1'
              && cb.getAttribute('data-belum-selesai') !== '1';
          }
        } else {
          cb.checked = match;
        }
  ...
  function loadSales(jenis) {
    ...
    if (forceComplete) {
      ...
      if (prefillSurcasId > 0) {
        q += '&exclude_surcas_id=' + encodeURIComponent(String(prefillSurcasId));
      }
    }
  ...
  document.getElementById('kurirSubmit').addEventListener('click', function () {
    ...
    if (forceComplete) {
      ...
      if (prefillSurcasId > 0) {
        fd.append('id_surcas', String(prefillSurcasId));
      }
    }
    ...
    if (jenis === 'antar' || jenis === 'jemput_antar') {
      ...
      var wantPending = wantPendingAntar();
  ...
  window.openKurirDelivery = function (jenis, noRef, jumlah, pengisi, idSurcas) {
    ...
    if (window.bootstrap && bootstrap.Offcanvas) {
      bootstrap.Offcanvas.getOrCreateInstance(root).show();
    }
  };
})();
</script>
