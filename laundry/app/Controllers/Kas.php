...
class Kas extends Controller
{
   ...
   public function index()
   {
      ...
      require_once 'app/Helper/SaldoNonTunai.php';
      $saldoMap = SaldoNonTunai::getSaldoMap($this->db(0), (int) $this->id_cabang);
      ...
      $this->view($view, [
         'saldo' => $saldo,
         'saldoBca' => (int) ($saldoMap['BCA'] ?? 0),
         'saldoQris' => (int) ($saldoMap['QRIS'] ?? 0),
   ...
   public function insert_penarikan_nontunai()
   {
      ...
      $min = ($note === 'QRIS') ? 1000 : 10000;
      if ($jumlah < $min) {
         header('HTTP/1.1 422 Unprocessable Entity');
         echo json_encode(['error' => 'Minimal penarikan Rp ' . number_format($min, 0, ',', '.')]);
         return;
      }

      require_once 'app/Helper/SaldoNonTunai.php';
      $saldoTersedia = SaldoNonTunai::getSaldo($this->db(0), (int) $this->id_cabang, $note);
      if ($jumlah > $saldoTersedia) {
         header('HTTP/1.1 422 Unprocessable Entity');
         echo json_encode([
            'error' => 'Saldo ' . $note . ' tidak cukup. Tersedia Rp ' . number_format($saldoTersedia, 0, ',', '.'),
         ]);
         return;
      }
