...
return new class extends Migration {
    ...
    public function up(): void
    {
        if (Schema::connection($this->connection)->hasColumn('users', 'reset_pass_token')) {
            DB::connection($this->connection)->statement('ALTER TABLE `users` CHANGE `reset_pass_token` `pin_reset` VARCHAR(100) NULL');
        }
    ...
    public function down(): void
    {
        ...
        if (Schema::connection($this->connection)->hasColumn('users', 'pin_reset')) {
            DB::connection($this->connection)->statement('ALTER TABLE `users` CHANGE `pin_reset` `reset_pass_token` VARCHAR(100) NULL');
        }
    }
};
