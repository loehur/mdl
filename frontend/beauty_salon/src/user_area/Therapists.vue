  ...
  <div class="max-w-5xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
      <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Terapis</h2>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm">
          ...
          <tbody v-else-if="therapists.length" class="divide-y">
            <tr v-for="(therapist, index) in therapists" :key="therapist.id" class="hover:bg-fuchsia-50/30 group">
              <td class="px-4 py-3 text-gray-500">{{ index + 1 }}</td>
              <td class="px-4 py-3 font-medium text-gray-800">{{ therapist.nama }}</td>
              <td class="px-4 py-3 text-gray-600">{{ therapist.no_hp }}</td>
              <td class="px-4 py-3 text-right">
                <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition">
                  <button @click="editTherapist(therapist)" class="text-blue-600 hover:text-blue-700 p-1">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                  </button>
                  <button @click="confirmDelete(therapist)" class="text-red-600 hover:text-red-700 p-1">
          ...
          <tbody v-else>
            <tr><td colspan="4" class="px-4 py-12 text-center text-gray-400">Belum ada data terapis</td></tr>
          </tbody>
        </table>
      </div>
    </div>
    ...
    <Teleport to="body">
      <div v-if="showModal" class="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
          <div class="bg-gradient-to-r from-pink-500 to-fuchsia-600 px-6 py-4">
            <h3 class="font-bold text-white text-lg">{{ editMode ? 'Edit Terapis' : 'Tambah Terapis' }}</h3>
          </div>
          <form @submit.prevent="saveTherapist" class="p-6 space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Nama <span class="text-red-500">*</span></label>
              <input v-model="form.nama" type="text" required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-pink-200 focus:border-pink-400 outline-none" placeholder="Nama terapis" />
            </div>
            <div>
      ...
      <div v-if="deleteConfirm.show" class="fixed inset-0 z-[100001] flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
          ...
          <div class="p-6"><p class="text-center text-gray-800">Hapus terapis <strong>{{ deleteConfirm.name }}</strong>?</p></div>
          <div class="px-6 pb-6 flex gap-3">
            <button @click="deleteConfirm.show = false" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg">Batal</button>
            <button @click="deleteTherapist" class="flex-1 px-4 py-2 bg-red-500 text-white rounded-lg">Hapus</button>
          </div>
        </div>
      </div>
...
const deleteConfirm = reactive({ show: false, id: null, name: '' });
...
async function fetchTherapists() {
  ...
  try {
    ...
    if (data.success) therapists.value = data.data;
  } catch (e) {
    console.error(e);
  } finally {
    loading.value = false;
...
async function saveTherapist() {
  saving.value = true;
  try {
    const url = editMode.value ? `/api/Beauty_Salon/Therapists/update/${form.id}` : '/api/Beauty_Salon/Therapists/create';
...
function confirmDelete(dt) {
  deleteConfirm.id = dt.id;
  deleteConfirm.name = dt.nama;
...
async function deleteTherapist() {
  ...
  try {
    const res = await fetch(`/api/Beauty_Salon/Therapists/delete/${id}`, { method: 'POST' });
    const data = await res.json();
    if (data.success) {
      showToast(data.message);
      fetchTherapists();
