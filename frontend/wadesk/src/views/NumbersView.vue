            ...
            <div v-for="n in numbers" :key="n.id" class="px-4 py-3">
              <div class="flex flex-wrap items-start justify-between gap-3">
                ...
                <div class="flex flex-wrap gap-1.5 justify-end text-xs">
                  <span class="px-2 py-1 rounded bg-white/5 text-slate-300">{{ n.status }}</span>
...
const logout=async()=>{await auth.logout();router.push({name:'login'})}; const qualityClass=q=>({GREEN:'bg-emerald-500/10 text-emerald-300',YELLOW:'bg-amber-500/10 text-amber-300',RED:'bg-rose-500/10 text-rose-300'}[String(q).toUpperCase()]||'bg-white/5 text-slate-300');
